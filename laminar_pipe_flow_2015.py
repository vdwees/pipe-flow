...
def initialize():
    ...
    # Duct Dimentions
    length = 0.02 # m, horizontal dimention, cooresponds to u
    width = 0.0125 # m, vertical dimention, cooresponds to v

    # Assume srid spacing equal in horizontal and vertical
    res = 120 # Grid Resoultion, points per width
    v_in = 0.0055765 # m/s, initial velocity u distributed on inlet and outlet

    # Psudo-speed of Sound (v_max / a < 1)
    a = 100 # m/s
...
def Re(p, rho, len_scale, v_in, nu):
    ...
    is calcualted using the equation:
...
def delta_t_max(delta_x, delta_y, a, num_dimentions = 2):
    """
    ...
    """
    return (2 * np.min([delta_x,delta_y]) /
           (np.sqrt(num_dimentions) * a * (1 + np.sqrt(5)) ))
...
def take_step(p, u, v, delta_t, Re, delta_x, delta_y, a):
    ...
    u2 = 0.25 * (u[1:,1:-1] + u[:-1,1:-1])**2 # u^2; has extra dimention in u
    v2 = 0.25 * (v[1:-1,1:] + v[1:-1,:-1])**2 # v^2; has extra dimention in v

    # These are sets of u*v values. uv1 has extra dimention in v
    ...
    p = p - delta_t * a**2 * ((u[1:,1:-1] - u[:-1,1:-1]) / delta_x +
                              (v[1:-1,1:] - v[1:-1,:-1]) / delta_y )
    return p, u, v


def u_theoretical(npts, v_avg, fullwidth=True):
    """
    This equation coems from:
...
def compare_u_profile(u, delta_y, width, v_in, Re):
    ...
    theoretically derived profile shape. This function takes a dimentional
    u and outputs results in dimentionalized form.
    """
...
def make_planes(length, width, res):
    ...
    # Non-dimentionalize distance terms by deviding by width,
    # Non-dimentionalize velocity terms by deviding by v_in
...
def infinite_planes(tol = 0.01):
    """
    This function takes one attibute, tol, the criteria for convergence. When
    ...
    while conv==False:
        ...
        continouity_factor = np.max(np.abs((p_old-p)/(delta_t * a**2)))
        if iter % 10000 == 0 :
            print('continouity_factor = %1.4f' % continouity_factor,
                  flush=True)
        if continouity_factor < tol:
            """
            ...
            print("Infinitized at %2.0f seconds," % duration,
                  iter, "iterations.", flush=True)
            infinitize.append(iter)

    # Report Calculation Statistics
    duration = time.time() - start_time
    print("Scheme took %2.0f seconds and" % duration, iter,
          "iterations to converge. tol:", tol )

    u, v, p, delta_x, delta_y = re_dim(u, v, p, delta_x, delta_y, width,
                                       rho, v_in)

    return u, v, p, delta_x, delta_y, width, length, v_in, np.max(Re_vec)

def re_dim(u, v, p, delta_x, delta_y, width, rho, v_in):
    """
    Re-dimentionalize output
    """
    u = u * v_in
    v = v * v_in
    p = p * rho * v_in**2
...
def widening_of_planes(tol = 0.001):
    ...
    while conv==False:
        ...
        continouity_factor = np.max(np.abs((p_old-p)/(delta_t * a**2)))
        if iter % 1000 == 0 :
            print('continouity_factor = %1.4f' % continouity_factor,', iter =',
                  iter, flush=True)
        if continouity_factor < tol:
            conv = True
    ...
    duration = time.time() - start_time
    print()
    print("Scheme took %4.0f seconds and" % duration, iter,
          "iterations to converge. tol:", tol)
...
def plot_streamplot(u, v, delta_x, delta_y, width, length, Re):
    ...
    ax.set_title('Streamplot of Fluid (Line Weight Proportional to Velocity),'
                 ' Re = %2.1f' % Re)
